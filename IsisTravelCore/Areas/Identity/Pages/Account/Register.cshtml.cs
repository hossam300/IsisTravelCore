...
namespace IsisTravelCore.Areas.Identity.Pages.Account
{
    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            {
                var user = new ApplicationUser { UserName = Input.UserName, Email = Input.Email, LockoutEnabled = true };
                var result = await _userManager.CreateAsync(user, Input.Password);
                if (result.Succeeded)
                {
                    if (!_roleManager.RoleExistsAsync("Administrator").Result)
                    {
                        var identityResult = await _roleManager.CreateAsync(new IdentityRole("Administrator"));
                    }
